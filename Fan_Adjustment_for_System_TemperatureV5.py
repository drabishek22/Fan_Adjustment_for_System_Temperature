...
import psutil  # Added to fetch CPU load
...
# Store fan speed changes for graphing and logging (limit to last 5 changes)
fan_speed_changes = []  # Will store tuples of (timestamp, fan_speed, reason)
...
def get_temp_info():
    result = subprocess.run(["ipmitool", "sdr", "type", "temperature"], capture_output=True, text=True)
    return result.stdout.strip()
...
def get_cpu_load():
    return psutil.cpu_percent(interval=1)  # Fetch CPU load over 1 second

# Function to set fan speed
def set_fan_speed(speed):
    subprocess.run(["ipmitool", "raw", "0x30", "0x30", "0x02", "0xff", f"{speed:02x}"])

    # Log the fan speed change with timestamp and reason
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    reason = f"Changed due to temperature threshold"
    fan_speed_changes.append((timestamp, speed, reason))
    
    # Limit to last 5 fan speed changes
    if len(fan_speed_changes) > 5:
        fan_speed_changes.pop(0)
...
def main(stdscr):
    ...
    while True:
        ...
        # Get fan, temperature, and CPU load information
        fan_info = get_fan_info().split('\n')
        temp_info = get_temp_info().split('\n')
        cpu_load = get_cpu_load()  # Get CPU load
        ...
        # Display CPU Load
        stdscr.addstr(1, 0, f"CPU Load: {cpu_load}%", curses.color_pair(1))
        ...
        # Adjust fan speed based on temperature and CPU load
        if highest_temp <= 38 and cpu_load <= 80:
            set_fan_speed(0x10)  # Lowest speed
        elif highest_temp <= 44 and cpu_load <= 80:
            set_fan_speed(0x20)  # New speed
        elif highest_temp <= 50 and cpu_load <= 80:
            set_fan_speed(0x30)  # Medium speed
        elif highest_temp <= 60 and cpu_load <= 80:
            set_fan_speed(0x40)  # New speed
        elif highest_temp <= 70 and cpu_load <= 80:
            set_fan_speed(0x50)  # High speed
        else:
            set_fan_speed(0x70)  # Maximum speed

        # If GPU temperature increases, ramp up the fan speed
        if gpu_temp and gpu_temp > 50:
            set_fan_speed(0x40, "GPU Overheating")  # Ramp fan to high speed if GPU temperature is high
